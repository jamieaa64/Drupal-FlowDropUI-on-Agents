...
class WorkflowParser {
  ...
  protected function createAgentComponent(array $node, array $edgeMap, array $workflowData): ?Component {
    ...
    $metadata = $workflowData['metadata']['agentConfig'] ?? [];

    // Get the agent ID from workflow data (this is the machine name).
    $agentId = $workflowData['id'] ?? $nodeId;
    ...
    $agentConfig = [
      ...
      'system_prompt' => $config['systemPrompt'] ?? $metadata['system_prompt'] ?? '',
      'max_loops' => (int) ($config['maxLoops'] ?? $metadata['max_loops'] ?? 3),
      'orchestration_agent' => (bool) ($config['orchestrationAgent'] ?? $metadata['orchestration_agent'] ?? FALSE),
      'triage_agent' => (bool) ($config['triageAgent'] ?? $metadata['triage_agent'] ?? FALSE),
